...
## 🎯 **Core Architecture Principles** ⚡ **PERFORMANCE CRITICAL**

### **🚫 FORBIDDEN PATTERNS - NEVER DO THESE**
1. **NO N+1 queries** - Always use bulk operations for multiple records
2. **NO `error_log()` calls** - Use controlled debug system (`oj_debug_log()`)
3. **NO nested loops with calculations** - Pre-calculate data outside loops
...
- **MANDATORY**: All services must use bulk operations and avoid N+1 queries
- **Example Pattern** (Performance Optimized):
```php
class Orders_Jet_Kitchen_Service {
    // ✅ CORRECT - Bulk operations for multiple orders
    public function get_orders_kitchen_data_bulk(array $order_ids): array
    public function get_kitchen_readiness_status_bulk(array $orders): array
    
    // ✅ ACCEPTABLE - Single order operations (when needed)
...
## ⚡ **PERFORMANCE OPTIMIZATION STANDARDS** (MANDATORY)

### **Database Query Optimization**
```php
// ❌ FORBIDDEN - N+1 Query Problem
foreach ($orders as $order) {
    $meta = $order->get_meta('_some_key'); // N+1 queries!
    $items = $order->get_items(); // N+1 queries!
    $user = get_userdata($order->get_meta('_assigned_user')); // N+1 queries!
}
...
$items_data = $this->get_orders_items_bulk($order_ids); // Single query
$users_data = $this->get_users_data_bulk($meta_data); // Single query

foreach ($orders as $order) {
    $meta = $meta_data[$order->get_id()] ?? array(); // Pre-fetched
    $items = $items_data[$order->get_id()] ?? array(); // Pre-fetched
    $user = $users_data[$meta['_assigned_user']] ?? null; // Pre-fetched
}
```

### **Logging System**
```php
// ❌ FORBIDDEN - Direct error_log (floods production logs)
error_log('Debug message');
error_log('Order processing: ' . print_r($data, true));

// ✅ REQUIRED - Controlled Debug System
oj_debug_log('Debug message', 'CONTEXT'); // Rate-limited, debug-only
oj_error_log('Error occurred', 'CONTEXT', $error_data); // Production errors
oj_perf_log('Operation completed', $execution_time_ms, 'PERFORMANCE');
```

### **Loop Optimization**
```php
// ❌ FORBIDDEN - Nested loops with calculations
foreach ($orders as $order) {
    foreach ($order->get_items() as $item) {
        $addon_total = $this->calculate_addon_total($item); // Expensive calculation in nested loop!
    }
}

// ✅ REQUIRED - Pre-calculate outside loops
Orders_Jet_Addon_Calculator::precalculate_addon_totals($order_ids); // Single calculation
foreach ($orders as $order) {
    $addon_total = Orders_Jet_Addon_Calculator::get_order_addon_total($order->get_id()); // Pre-calculated
}
```

### **Caching Strategy**
```php
// ✅ REQUIRED - Intelligent caching with appropriate durations
$cache_duration = $this->determine_cache_duration($filter, $search, $data_volatility);
set_transient($cache_key, $data, $cache_duration);

// Cache durations based on data characteristics:
// - Completed orders: 5 minutes (stable data)
// - Active orders: 1 minute (frequently changing)
// - Search results: 1 minute (dynamic queries)
```
