...
define('ORDERS_JET_VERSION', '1.0.1');
...
$dependencies_autoload = ORDERS_JET_PLUGIN_DIR . 'vendor/autoload.php';
if (file_exists($dependencies_autoload)) {
    require_once $dependencies_autoload;
}

// Pusher Realtime Notifications Constants
// Set these in wp-config.php to override defaults, or leave empty to disable Pusher
if (!defined('ORDERS_JET_PUSHER_ENABLED')) {
    define('ORDERS_JET_PUSHER_ENABLED', true);
}
if (!defined('ORDERS_JET_PUSHER_KEY')) {
    define('ORDERS_JET_PUSHER_KEY', '6030805af5d148dd2fbd');
}
if (!defined('ORDERS_JET_PUSHER_SECRET')) {
    define('ORDERS_JET_PUSHER_SECRET', '5c7446f4c6c0aca44d83');
}
if (!defined('ORDERS_JET_PUSHER_APP_ID')) {
    define('ORDERS_JET_PUSHER_APP_ID', '2079029');
}
if (!defined('ORDERS_JET_PUSHER_CLUSTER')) {
    define('ORDERS_JET_PUSHER_CLUSTER', 'eu');
}
if (!defined('ORDERS_JET_PUSHER_USE_TLS')) {
    define('ORDERS_JET_PUSHER_USE_TLS', true);
}
...
class Orders_Jet_Integration {
    ...
    private function includes(): void {
        // Utility classes (loaded first - used by other classes)
        include_once ORDERS_JET_PLUGIN_DIR . 'includes/utilities/class-woojet-meta-keys.php';
        include_once ORDERS_JET_PLUGIN_DIR . 'includes/utilities/class-woojet-datetime-helper.php';
        
        // Service classes (Phase 2 refactoring)
        include_once ORDERS_JET_PLUGIN_DIR . 'includes/class-orders-jet-logger.php';
        include_once ORDERS_JET_PLUGIN_DIR . 'includes/class-orders-jet-addon-calculator.php';
        include_once ORDERS_JET_PLUGIN_DIR . 'includes/class-orders-jet-business-insights.php';
        ...
        include_once ORDERS_JET_PLUGIN_DIR . 'includes/services/class-orders-jet-kitchen-filter-service.php';
        include_once ORDERS_JET_PLUGIN_DIR . 'includes/services/class-orders-jet-notification-service.php';
        include_once ORDERS_JET_PLUGIN_DIR . 'includes/services/class-orders-jet-realtime-service.php';
        include_once ORDERS_JET_PLUGIN_DIR . 'includes/services/class-orders-jet-order-method-service.php';
        include_once ORDERS_JET_PLUGIN_DIR . 'includes/services/class-orders-jet-query-service.php';
        include_once ORDERS_JET_PLUGIN_DIR . 'includes/services/class-orders-jet-amount-filter-service.php';
        include_once ORDERS_JET_PLUGIN_DIR . 'includes/services/class-orders-jet-saved-views-service.php';
        ...
        include_once ORDERS_JET_PLUGIN_DIR . 'includes/handlers/class-orders-jet-notification-handler.php';
        include_once ORDERS_JET_PLUGIN_DIR . 'includes/handlers/class-orders-jet-bulk-actions-handler.php';
        include_once ORDERS_JET_PLUGIN_DIR . 'includes/handlers/class-orders-jet-dashboard-refresh-handler.php';
        include_once ORDERS_JET_PLUGIN_DIR . 'includes/handlers/class-orders-jet-order-details-handler.php';
        include_once ORDERS_JET_PLUGIN_DIR . 'includes/handlers/class-orders-jet-filter-views-handler.php';
        include_once ORDERS_JET_PLUGIN_DIR . 'includes/handlers/class-orders-jet-order-editor-handler.php';
        include_once ORDERS_JET_PLUGIN_DIR . 'includes/handlers/class-orders-jet-table-assignment-handler.php';
        ...
        include_once ORDERS_JET_PLUGIN_DIR . 'includes/components/notification-center.php';
        
        // Helpers
        include_once ORDERS_JET_PLUGIN_DIR . 'includes/helpers/time-helpers.php';
        ...
        include_once ORDERS_JET_PLUGIN_DIR . 'includes/class-orders-jet-capabilities.php'; // NEW: WordPress-native capability system
        ...
        include_once ORDERS_JET_PLUGIN_DIR . 'includes/helpers/class-orders-jet-filter-url-builder.php';
        include_once ORDERS_JET_PLUGIN_DIR . 'includes/classes/class-orders-master-query-builder.php';
    ...
    private function init_components(): void {
        // Initialize capabilities system (WordPress-native approach)
        new Orders_Jet_Capabilities();
        
        // Initialize user roles (legacy - will be deprecated)
        ...
        // Initialize Order Editor Handler (Phase 2: Add Notes)
        // Create required services
        $tax_service = new Orders_Jet_Tax_Service();
        ...
        $factory = new Orders_Jet_Handler_Factory($tax_service, $kitchen_service, $notification_service);
        
        // Initialize Order Editor Handler (registers AJAX endpoints)
        $factory->get_order_editor_handler();
    ...
    public function display_table_menu(): void {
        try {
            ...
            // Get menu items (WooCommerce products) - OPTIMIZED
            $menu_items = wc_get_products(array(
                'status' => 'publish',
                'limit' => 500, // Reasonable limit for menu items
                'orderby' => 'menu_order',
                'order' => 'ASC'
            ));
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) {
                oj_error_log('Table menu error: ' . $e->getMessage(), 'INTEGRATION');
            }
...
function oj_set_default_order_status(int $order_id): void {
    ...
    if ($order && !$order->get_meta('_oj_order_status')) {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            oj_debug_log('Setting default order status for order #' . $order_id . ' - Type: ' . $order_type . ', Status: ' . $default_status, 'INTEGRATION');
        }
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            oj_debug_log('Successfully set order status for order #' . $order_id, 'INTEGRATION');
        }
    }
}
...
function oj_set_existing_orders_status(): void {
    ...
    if (!empty($orders)) {
        foreach ($orders as $order) {
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) {
                oj_debug_log('Retroactively set order status for order #' . $order->get_id() . ' - Type: ' . $order_type . ', Status: ' . $default_status, 'INTEGRATION');
            }
        }
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            oj_debug_log('Retroactively processed ' . count($orders) . ' orders without _oj_order_status', 'INTEGRATION');
        }
    }
}
