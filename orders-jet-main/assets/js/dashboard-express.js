...
// Define showExpressNotification globally BEFORE document ready
// This ensures it's available for the notification center immediately
window.showExpressNotification = function(message, type = 'success') {
...
jQuery(document).ready(function($) {
    ...
    $document.on('click', '.oj-mark-ready, .oj-mark-ready-food, .oj-mark-ready-beverage', function(e) {
        ...
        const $btn = $(this);
        const $card = $btn.closest('.oj-order-card, .oj-kitchen-card'); // Support both manager and kitchen cards
        ...
        $.ajax({
            ...
            success: function(response) {
                // Check if this is a kitchen user FIRST - before any other processing
                const isKitchenUser = $('body').hasClass('oj-kitchen-user') || $('.oj-kitchen-card').length > 0;
                
                if (isKitchenUser) {
                    // Kitchen interface - remove card immediately regardless of partial/full ready
                    $card.fadeOut(300, function() {
                        $(this).remove();
                        
                        // Update counts using real-time data from backend
                        if (response.data && response.data.updated_counts) {
                            updateKitchenCounts(response.data.updated_counts);
                        } else {
                            // Fallback: simple count decrement
                            const $counter = $('.oj-page-stats .oj-count-text, .oj-count-text');
                            if ($counter.length) {
                    ...
                    return; // Exit early for kitchen users - no further processing needed
                }
                
                // Manager interface processing - only if NOT kitchen user
                if (response.success && response.data.card_updates) {
                    ...
                    if (updates.partial_ready) {
                        ...
                        // Manager interface - replace all buttons with completion button
    ...
    // OLD Print Invoice handler removed - duplicate causing double print dialog
    // New handler is further down (line ~1017) with better implementation
    ...
    // showExpressNotification is now defined globally at the top of the file
    // This ensures it's available immediately for the notification center
    ...
    $(document).on('click', '.oj-print-invoice', function(e) {
        ...
        // Use hidden iframe for thermal printing (NO new tab)
        const iframe = $('<iframe>', {
            src: thermalInvoiceUrl,
            style: 'display:none'
        });
        ...
        // Update button to "Paid?" state immediately
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        setTimeout(() => {
                            ...
                            // Trigger grid refresh for waiter view to update counts
                            if (window.ojNotificationCenter && typeof window.ojNotificationCenter.refreshOrdersGrid === 'function') {
                                window.ojNotificationCenter.refreshOrdersGrid();
                            }
                        }, 500);
    ...
    // ========================================================================
    // SIMPLE REFRESH FUNCTION - Just reload the current page content
    // ========================================================================
    
    window.refreshKitchenDashboard = function() {
        ...
        $.ajax({
            ...
            success: function(html) {
                ...
                // Extract and update filter counts
    ...
    // Helper function to get current user's kitchen type
    function getUserKitchenType() {
        // Check if we're on a specific kitchen page or have kitchen type data
        if (typeof ojExpressData !== 'undefined' && ojExpressData.userKitchenType) {
            return ojExpressData.userKitchenType;
        }
        
        // Fallback: detect from page context or URL
        const urlParams = new URLSearchParams(window.location.search);
        const pageParam = urlParams.get('page');
        
        if (pageParam && pageParam.includes('kitchen')) {
            return 'food'; // Default to food kitchen
        }
        
        // Check for kitchen specialization in body classes or data attributes
        if ($('body').hasClass('oj-food-kitchen')) return 'food';
        if ($('body').hasClass('oj-beverage-kitchen')) return 'beverages';
    ...
    function updateLastUpdatedTime() {
        ...
        const $lastUpdated = $('.oj-last-updated');
        
        if ($lastUpdated.length) {
            $lastUpdated.html(
                '<span class="dashicons dashicons-clock" style="font-size: 14px; vertical-align: middle; margin-right: 4px;"></span>' +
                '<strong>Last Updated:</strong> ' + timeString
            );
        }
    }
    
    // Helper function to update filter counts (reuse existing function if available)
    function updateExpressFilterCounts(counts) {
        if (typeof counts === 'object') {
            ...
            const totalCount = counts.active || counts.food_kitchen || counts.beverage_kitchen || 0;
            $('.oj-count-text').text(totalCount + ' orders');
        }
    }
    
    // Kitchen-specific count updates with animations
    function updateKitchenCounts(counts) {
        if (typeof counts === 'object') {
            // Update filter button counts with smooth animation
            $('.oj-filter-btn[data-filter="active"] .oj-count').fadeOut(150, function() {
                $(this).text(counts.active || 0).fadeIn(150);
            });
            $('.oj-filter-btn[data-filter="food_kitchen"] .oj-count').fadeOut(150, function() {
                $(this).text(counts.food_kitchen || 0).fadeIn(150);
            });
            $('.oj-filter-btn[data-filter="beverage_kitchen"] .oj-count').fadeOut(150, function() {
                $(this).text(counts.beverage_kitchen || 0).fadeIn(150);
            });
            
            // Update header count with animation - SIMPLE: Just count visible cards
            const $counter = $('.oj-kitchen-count, .oj-count-text');
            if ($counter.length) {
                // Simple approach: Count visible order cards after the action
                setTimeout(function() {
                    const visibleCards = $('.oj-order-card:visible, .oj-kitchen-card:visible').length;
                    const countText = visibleCards === 1 ? visibleCards + ' order' : visibleCards + ' orders';
                    
                    $counter.fadeOut(150, function() {
                        $(this).text(countText).fadeIn(150);
                    });
                }, 350); // Wait for card removal animation to complete
            }
            
            // Update any other count displays
            $('.oj-filter-count').each(function() {
                const filter = $(this).closest('.oj-filter-btn').data('filter');
                if (counts[filter] !== undefined) {
                    $(this).fadeOut(150, function() {
                        $(this).text(counts[filter]).fadeIn(150);
                    });
                }
            });
        }
