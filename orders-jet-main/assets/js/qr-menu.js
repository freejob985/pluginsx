...
(function() {
    ...
    function initCallWaiterButton() {
        const callWaiterBtn = document.getElementById('waiter-call-button');
    ...
    function callWaiter() {
        const callButton = document.getElementById('waiter-call-button');
        const waiterIcon = callButton.querySelector('.waiter-icon');
        const waiterText = callButton.querySelector('.waiter-text');
        
        // Debug: Check if OrdersJetQRMenu is available
        console.log('OrdersJetQRMenu object:', window.OrdersJetQRMenu);
        
        if (!window.OrdersJetQRMenu) {
            console.error('OrdersJetQRMenu configuration not found');
            showAppNotification('Configuration error. Please refresh the page.', 'error');
            return;
        }
        
        if (!window.OrdersJetQRMenu.tableNumber) {
            console.error('Table number not found in configuration');
            showAppNotification('Table number not found. Please refresh the page.', 'error');
            return;
        }
        
        // Show loading state
        callButton.classList.add('loading');
        waiterIcon.textContent = '⏳';
        waiterText.textContent = 'Calling...';
        
        // Prepare call data
        const callData = {
            table_number: window.OrdersJetQRMenu.tableNumber,
            timestamp: new Date().toISOString(),
            message: 'Guest is requesting waiter assistance'
        };
        ...
        const requestData = {
            action: 'oj_call_waiter',
            call_data: JSON.stringify(callData),
            nonce: window.OrdersJetQRMenu.nonces.call_waiter || window.OrdersJetQRMenu.nonces.table_order
        };
        
        console.log('Request data:', requestData);
        console.log('AJAX URL:', window.OrdersJetQRMenu.ajaxUrl);
        ...
        .then(data => {
            if (data.success) {
                ...
                waiterIcon.textContent = '✅';
                waiterText.textContent = 'Called!';
                
                showAppNotification('Waiter has been notified! They will be with you shortly.', 'success');
                
                // Reset button after 3 seconds
                setTimeout(() => {
                    callButton.classList.remove('success');
                    waiterIcon.textContent = '🙋‍♂️';
                    waiterText.textContent = 'Call Waiter';
                }, 3000);
                ...
                callButton.classList.remove('loading');
                waiterIcon.textContent = '❌';
                waiterText.textContent = 'Failed';
                
                showAppNotification('Failed to call waiter: ' + (data.data?.message || 'Please try again'), 'error');
        ...
        .catch(error => {
            ...
            console.error('Error details:', {
                ...
                ajaxUrl: window.OrdersJetQRMenu.ajaxUrl
            });
            
            // Show error state
            callButton.classList.remove('loading');
            waiterIcon.textContent = '❌';
            waiterText.textContent = 'Error';
            
            showAppNotification('Error calling waiter: ' + error.message, 'error');
    ...
    window.testCallWaiter = function() {
        console.log('Testing call waiter functionality...');
        console.log('Configuration:', window.OrdersJetQRMenu);
        
        if (!window.OrdersJetQRMenu) {
            console.error('OrdersJetQRMenu not found!');
            return;
        }
        ...
        const testData = {
            ...
            call_data: JSON.stringify({
                table_number: window.OrdersJetQRMenu.tableNumber,
                timestamp: new Date().toISOString(),
                message: 'Test call from console'
            }),
            nonce: window.OrdersJetQRMenu.nonces.call_waiter || window.OrdersJetQRMenu.nonces.table_order
        };
        ...
        .then(text => {
            ...
            try {
                ...
                console.error('Test response is not JSON:', e);
            }
        })
        .catch(error => {
