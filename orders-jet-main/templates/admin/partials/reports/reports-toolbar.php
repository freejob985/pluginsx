<?php
/**
 * Orders Reports - Single Row Toolbar
 * 
...
<div class="oj-master-toolbar">
    <form method="get" action="" class="oj-toolbar-form">
        ...
        <!-- Table Filter Dropdown (uses existing search logic) -->
        <div class="oj-toolbar-group oj-search-group">
            ...
            if (class_exists('Orders_Master_Query_Builder') && method_exists('Orders_Master_Query_Builder', 'get_available_tables')) {
                try {
                    ...
                    ?>
                    <select name="search" class="oj-toolbar-select oj-search-select oj-table-select" onchange="updateToolbarFilter(this)">
                        <option value=""><?php _e('🏠 All Tables', 'orders-jet'); ?></option>
                        <?php
                        foreach ($available_tables as $table) {
                            $selected = ($current_search === $table['value']) ? 'selected' : '';
                            echo '<option value="' . esc_attr($table['value']) . '" ' . $selected . '>' . esc_html($table['label']) . '</option>';
                        }
                        ?>
                    </select>
                    <?php
                    ...
                    <input type="text" 
                           ...
                           placeholder="🔍 Search orders... (fallback)"
                ...
                <input type="text" 
                       name="search" 
                       value="<?php echo esc_attr($search); ?>"
                       placeholder="🔍 Search orders... (class not found)"
                       class="oj-toolbar-input oj-search-input">
                <?php
            }
            ?>
        ...
        <div class="oj-toolbar-group oj-sort-group">
            <select name="orderby_order" class="oj-toolbar-select oj-sort-select" onchange="updateToolbarFilter(this)">
                <?php
                $current_sort = ($orderby ?? 'date_created') . '_' . ($order ?? 'DESC');
                ?>
                <option value="date_created_DESC" <?php selected($current_sort, 'date_created_DESC'); ?>>📈 Newest First</option>
                <option value="date_created_ASC" <?php selected($current_sort, 'date_created_ASC'); ?>>📈 Oldest First</option>
                <option value="date_modified_DESC" <?php selected($current_sort, 'date_modified_DESC'); ?>>🔄 Recently Updated</option>
                <option value="date_modified_ASC" <?php selected($current_sort, 'date_modified_ASC'); ?>>🔄 Oldest Updated</option>
                <option value="total_DESC" <?php selected($current_sort, 'total_DESC'); ?>>💰 Highest Value</option>
                <option value="total_ASC" <?php selected($current_sort, 'total_ASC'); ?>>💰 Lowest Value</option>
            </select>
        </div>
        ...
        <div class="oj-toolbar-group oj-actions-group">
            ...
            <a href="<?php echo admin_url('admin.php?page=orders-reports'); ?>" class="button oj-reset-btn">Reset</a>
