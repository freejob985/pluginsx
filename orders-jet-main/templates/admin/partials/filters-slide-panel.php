    ...
    <div class="oj-filters-panel">
        ...
        <div class="oj-filters-body">
            ...
            <div id="oj-tab-filters" class="oj-tab-content active">
                ...
                <!-- Search Filter -->
                <div class="oj-filter-group">
                    <label><?php _e('🔍 Search:', 'orders-jet'); ?></label>
                    <input type="text" id="panel-search" class="oj-filter-input" data-param="search" 
                           placeholder="<?php _e('Search orders, customers, table numbers...', 'orders-jet'); ?>">
                </div>
            ...
            <div class="oj-filter-group">
                ...
                <select id="panel-assigned-waiter" class="oj-filter-select" data-param="assigned_waiter">
                    <option value=""><?php _e('All Orders', 'orders-jet'); ?></option>
                    <option value="assigned_only"><?php _e('👥 All Waiters (Assigned)', 'orders-jet'); ?></option>
                    <option value="unassigned"><?php _e('🔍 Unassigned Orders', 'orders-jet'); ?></option>
                    <?php
                    // Get staff members from actual order assignments (same as BI system)
                    global $wpdb;
                    
                    // Get unique waiter IDs from order meta
                    $waiter_ids = $wpdb->get_col("
                        SELECT DISTINCT meta_value 
                        FROM {$wpdb->postmeta} 
                        WHERE meta_key = '_oj_assigned_waiter' 
                        AND meta_value != '' 
                    ...
                    if (!empty($waiter_ids)) {
                        foreach ($waiter_ids as $waiter_id) {
        ...
        <div class="oj-filters-footer">
            ...
            <a href="<?php echo admin_url('admin.php?page=orders-master-v2'); ?>" class="oj-btn oj-btn-secondary">
