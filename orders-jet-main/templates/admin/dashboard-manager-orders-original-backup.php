...
// Get total count for pagination - OPTIMIZED COUNT
...
// Count orders for filter tabs - OPTIMIZED COUNTS
$processing_result = wc_get_orders(array(
    'status' => 'wc-processing',
    'limit' => 1,
    'paginate' => true
));
$processing_count = $processing_result->total;

$pending_result = wc_get_orders(array(
    'status' => 'wc-pending', 
    'limit' => 1,
    'paginate' => true
));
...
// OPTIMIZED: Count by order type using direct database queries with caching
// FIXED: Eliminates the need to fetch 500+ orders just for counting

// Check cache first for operational filter counts
$cache_key = 'oj_manager_orders_operational_counts';
$operational_counts = get_transient($cache_key);

if ($operational_counts === false) {
    global $wpdb;
    
    // Count dine-in orders (orders with table numbers or exwf_odmethod = 'dinein')
    $dinein_count = $wpdb->get_var("
        SELECT COUNT(DISTINCT p.ID) 
        FROM {$wpdb->posts} p
        LEFT JOIN {$wpdb->postmeta} pm_method ON p.ID = pm_method.post_id AND pm_method.meta_key = 'exwf_odmethod'
        ...
        AND (pm_method.meta_value = 'dinein' OR pm_table.meta_value IS NOT NULL)
    ");

    // Count takeaway orders (orders with exwf_odmethod = 'takeaway' OR no method and no table)
    $takeaway_count = $wpdb->get_var("
        SELECT COUNT(DISTINCT p.ID) 
        FROM {$wpdb->posts} p
        LEFT JOIN {$wpdb->postmeta} pm_method ON p.ID = pm_method.post_id AND pm_method.meta_key = 'exwf_odmethod'
        LEFT JOIN {$wpdb->postmeta} pm_table ON p.ID = pm_table.post_id AND pm_table.meta_key = '_oj_table_number'
        WHERE p.post_type = 'shop_order'
        AND p.post_status IN ('wc-processing', 'wc-pending')
        AND (pm_method.meta_value = 'takeaway' 
             OR (pm_method.meta_value IS NULL AND pm_table.meta_value IS NULL))
        AND p.ID NOT IN (
            SELECT DISTINCT p2.ID 
            FROM {$wpdb->posts} p2
            LEFT JOIN {$wpdb->postmeta} pm_method2 ON p2.ID = pm_method2.post_id AND pm_method2.meta_key = 'exwf_odmethod'
            LEFT JOIN {$wpdb->postmeta} pm_table2 ON p2.ID = pm_table2.post_id AND pm_table2.meta_key = '_oj_table_number'
            WHERE p2.post_type = 'shop_order'
            AND p2.post_status IN ('wc-processing', 'wc-pending')
            AND (pm_method2.meta_value = 'dinein' OR pm_table2.meta_value IS NOT NULL OR pm_method2.meta_value = 'delivery')
        )
    ");

    // Count delivery orders (orders with exwf_odmethod = 'delivery')
    $delivery_count = $wpdb->get_var("
        SELECT COUNT(DISTINCT p.ID) 
        FROM {$wpdb->posts} p
        LEFT JOIN {$wpdb->postmeta} pm_method ON p.ID = pm_method.post_id AND pm_method.meta_key = 'exwf_odmethod'
        WHERE p.post_type = 'shop_order'
        AND p.post_status IN ('wc-processing', 'wc-pending')
        AND pm_method.meta_value = 'delivery'
    ");

    // Ensure counts are integers and handle null results
    $operational_counts = array(
        'dinein' => intval($dinein_count ?: 0),
        'takeaway' => intval($takeaway_count ?: 0),
        'delivery' => intval($delivery_count ?: 0)
    );
    
    // Cache for 60 seconds (balance between performance and accuracy)
    set_transient($cache_key, $operational_counts, 60);
} else {
    // Use cached values
    $dinein_count = $operational_counts['dinein'];
    $takeaway_count = $operational_counts['takeaway'];
    $delivery_count = $operational_counts['delivery'];
}
?>
