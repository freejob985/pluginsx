...
if (isset($_POST['oj_mark_ready']) && isset($_POST['order_id']) && isset($_POST['_wpnonce'])) {
    oj_debug_log('Form submission detected - Order ID: ' . $_POST['order_id'], 'KITCHEN_FORM');
    
    // Verify nonce
    if (wp_verify_nonce($_POST['_wpnonce'], 'oj_mark_ready_' . $_POST['order_id'])) {
        oj_debug_log('Nonce verified successfully', 'KITCHEN_FORM');
        
        // Check permissions
        if (current_user_can('access_oj_kitchen_dashboard') || current_user_can('manage_options')) {
            oj_debug_log('User permissions verified', 'KITCHEN_FORM');
            
            $order_id = intval($_POST['order_id']);
            $order = wc_get_order($order_id);
            
            oj_debug_log('Order retrieval - Order ID: ' . $order_id . ', Order object: ' . ($order ? 'Found' : 'NOT FOUND'), 'KITCHEN_FORM');
            
            if ($order) {
                $table_number = $order->get_meta('_oj_table_number');
                oj_debug_log('Table number: ' . ($table_number ?: 'None (pickup order)'), 'KITCHEN_FORM');
                
                // Allow both table orders AND pickup orders (no table number required)
                $current_status = $order->get_status();
                oj_debug_log('Current order status: ' . $current_status, 'KITCHEN_FORM');
                
                if (in_array($current_status, array('pending', 'processing'))) {
                    oj_debug_log('Status is valid for marking ready, attempting to change status...', 'KITCHEN_FORM');
                    ...
                    try {
                        ...
                        oj_debug_log('Status change attempt - Old: ' . $old_status . ', New: ' . $new_status . ', Save result: ' . $save_result, 'KITCHEN_FORM');
                        
                        if ($new_status === 'pending') {
                            oj_debug_log('SUCCESS - Order #' . $order_id . ' status changed to pending by user #' . get_current_user_id(), 'KITCHEN_FORM');
                            ...
                            oj_error_log('Status did not change as expected. Expected: pending, Actual: ' . $new_status, 'KITCHEN_FORM');
                            $error_message = sprintf(__('Status change failed. Expected: pending, Got: %s', 'orders-jet'), $new_status);
                        }
                        
                    } catch (Exception $e) {
                        oj_error_log('EXCEPTION during status change: ' . $e->getMessage(), 'KITCHEN_FORM');
                        oj_debug_log('Exception trace: ' . $e->getTraceAsString(), 'KITCHEN_FORM');
                        $error_message = __('Failed to mark order as ready. Error: ', 'orders-jet') . $e->getMessage();
                    }
                } else {
                    oj_debug_log('Invalid status for marking ready: ' . $current_status, 'KITCHEN_FORM');
                ...
                oj_error_log('Order not found for ID: ' . $order_id, 'KITCHEN_FORM');
                $error_message = __('Order not found.', 'orders-jet');
            }
        } else {
            oj_debug_log('User permission denied for user #' . get_current_user_id(), 'KITCHEN_FORM');
            $error_message = __('You do not have permission to perform this action.', 'orders-jet');
        }
    } else {
        oj_debug_log('Nonce verification failed', 'KITCHEN_FORM');
...
// OPTIMIZED: Get active orders with caching for kitchen dashboard performance
$cache_key = 'oj_kitchen_dashboard_active_orders';
$wc_orders = get_transient($cache_key);

if ($wc_orders === false && function_exists('wc_get_orders')) {
    oj_debug_log('Using ultra-simple approach with caching...', 'KITCHEN_ORDERS');
    
    // OPTIMIZED: Limit kitchen orders to reasonable number for performance
    $wc_orders = wc_get_orders(array(
        'status' => array('processing', 'in-progress'), // Cover both possible status names
        'limit' => 100, // Reasonable limit for kitchen display
        'orderby' => 'date',
        'order' => 'ASC'
    ));
    
    // Cache for 30 seconds (balance between performance and real-time updates)
    set_transient($cache_key, $wc_orders, 30);
}

if ($wc_orders !== false) {
    oj_debug_log('Found ' . count($wc_orders) . ' orders', 'KITCHEN_ORDERS');
    ...
    foreach ($wc_orders as $wc_order) {
        ...
        if (empty($table_number)) {
            oj_debug_log('Order #' . $wc_order->get_id() . ' - All meta fields:', 'KITCHEN_DEBUG');
            ...
            foreach ($all_meta as $meta) {
                ...
                if (strpos($key, 'delivery') !== false || strpos($key, 'exwf') !== false || strpos($key, 'date') !== false || strpos($key, 'time') !== false) {
                    oj_debug_log($key . ' = ' . print_r($value, true), 'KITCHEN_DEBUG');
                }
            }
        }
        ...
        oj_debug_log('Order #' . $wc_order->get_id() . ' - Delivery info: ' . ($has_delivery_time ? json_encode($delivery_info) : 'NONE'), 'KITCHEN_DEBUG');
        ...
        if (!empty($table_number)) {
            ...
            oj_debug_log('TIMED pickup order #' . $wc_order->get_id() . ' - ' . $order_type_label, 'KITCHEN_ORDERS');
        } else {
            ...
            oj_debug_log('REGULAR pickup order #' . $wc_order->get_id(), 'KITCHEN_ORDERS');
        }
    ...
    // Simple fallback
    oj_debug_log('Using simple fallback...', 'KITCHEN_ORDERS');
    
    // OPTIMIZED: Limit kitchen orders to reasonable number for performance
    $active_orders_posts = get_posts(array(
        'post_type' => 'shop_order',
        'post_status' => array('wc-processing', 'wc-in-progress'),
        'posts_per_page' => 100, // Reasonable limit for kitchen display
        'orderby' => 'date',
        'order' => 'ASC'
    ));
...
oj_debug_log('Final order count: ' . count($active_orders), 'KITCHEN_ORDERS');
oj_debug_log('Order IDs: ' . implode(', ', array_column($active_orders, 'ID')), 'KITCHEN_ORDERS');
...
if (function_exists('wc_get_orders')) {
    // OPTIMIZED COUNT: Use pagination for counting ready orders
    $completed_result = wc_get_orders(array(
        'status' => array('pending'), // Ready orders
        'limit' => 1,
        'paginate' => true
    ));
    $completed_orders = $completed_result->total;
} else {
    // OPTIMIZED: Use reasonable limit for counting completed orders
    $completed_orders_posts = get_posts(array(
        'post_type' => 'shop_order',
        'post_status' => array('wc-pending'),
        'posts_per_page' => 200 // Reasonable limit for counting
    ));
    $completed_orders = count($completed_orders_posts);
}

// Debug logging
oj_debug_log('In Progress Orders = ' . $in_progress_orders, 'KITCHEN_STATS');
oj_debug_log('Completed Orders = ' . $completed_orders, 'KITCHEN_STATS');
oj_debug_log('Active Orders Count = ' . count($active_orders), 'KITCHEN_STATS');

// Additional debug for completed orders
if (function_exists('wc_get_orders')) {
    oj_debug_log('Using wc_get_orders for completed orders', 'KITCHEN_STATS');
    if (isset($completed_wc_orders)) {
        oj_debug_log('Found ' . count($completed_wc_orders) . ' pending orders', 'KITCHEN_STATS');
        foreach ($completed_wc_orders as $completed_order) {
            oj_debug_log('On-hold Order #' . $completed_order->get_id() . ' - Status: ' . $completed_order->get_status() . ' - Table: ' . $completed_order->get_meta('_oj_table_number'), 'KITCHEN_STATS');
        }
    }
} else {
    oj_debug_log('Using get_posts fallback for completed orders', 'KITCHEN_STATS');
}
...
<div class="wrap">
    <div class="oj-header-container">
        ...
        <div class="oj-header-right">
            <button type="button" class="oj-check-orders-btn" onclick="refreshKitchenDashboard();">
                <span class="dashicons dashicons-update"></span>
                <?php _e('Check for New Orders', 'orders-jet'); ?>
            </button>
            <div class="oj-refresh-indicator" style="display: none; margin-left: 10px; color: #0073aa; font-size: 12px;">
                ...
                <?php elseif ($order['post_status'] === 'wc-processing') : ?>
                    ...
                    if ($delivery_info) {
                        ...
                        if ($countdown_info) {
                            ...
                            oj_debug_log('Order #' . $order['ID'] . ' Countdown Debug:', 'KITCHEN_COUNTDOWN');
                            oj_debug_log('- Current time: ' . date('Y-m-d H:i:s', $countdown_info['current_time']), 'KITCHEN_COUNTDOWN');
                            oj_debug_log('- Delivery time: ' . date('Y-m-d H:i:s', $countdown_info['delivery_time']), 'KITCHEN_COUNTDOWN');
                            oj_debug_log('- Diff seconds: ' . $countdown_info['diff_seconds'], 'KITCHEN_COUNTDOWN');
                            oj_debug_log('- Status: ' . $countdown_info['status'], 'KITCHEN_COUNTDOWN');
                            oj_debug_log('- Text: ' . $countdown_text, 'KITCHEN_COUNTDOWN');
                            ?>
