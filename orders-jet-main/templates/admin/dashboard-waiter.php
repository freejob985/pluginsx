...
// OPTIMIZED: Bulk fetch all active orders for all tables in ONE query with caching
// FIXED: Eliminates N+1 query problem by fetching all data upfront
$table_numbers = array_column($tables, 'post_title');

if (!empty($table_numbers)) {
    // Check cache first for waiter dashboard data
    $cache_key = 'oj_waiter_dashboard_orders_' . md5(implode(',', $table_numbers));
    $orders_by_table = get_transient($cache_key);
    
    if ($orders_by_table === false) {
        // Single optimized query to get all active orders for all tables
        $all_orders = wc_get_orders(array(
            ...
            'limit' => 200, // Reasonable limit for waiter dashboard
            'orderby' => 'date',
            'order' => 'DESC',
            'return' => 'objects' // Get WC_Order objects directly
        ));
        
        // Group orders by table number for efficient lookup
        $orders_by_table = array();
        foreach ($all_orders as $order) {
            $table_number = $order->get_meta('_oj_table_number');
            if (!isset($orders_by_table[$table_number])) {
                $orders_by_table[$table_number] = array();
            }
        ...
        set_transient($cache_key, $orders_by_table, 30);
    }
} else {
    $orders_by_table = array();
}

// OPTIMIZED: Process tables with pre-fetched order data (no additional queries)
foreach ($tables as &$table) {
    $table_number = $table['post_title'];
...
$orders_served_today = $wpdb->get_var($wpdb->prepare("
    SELECT COUNT(*) 
    FROM {$wpdb->posts} p
    LEFT JOIN {$wpdb->postmeta} pm_waiter ON p.ID = pm_waiter.post_id AND pm_waiter.meta_key = '_oj_assigned_waiter'
