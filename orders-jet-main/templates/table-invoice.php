...
// OPTIMIZED: Get completed orders with caching and reasonable limit
$cache_key = 'oj_invoice_orders_' . sanitize_key($table_number);
...
if ($orders === false) {
    $orders = wc_get_orders(array(
        ...
        'limit' => 50, // Reasonable limit for invoice orders
        'orderby' => 'date',
        'order' => 'DESC',
        'return' => 'objects' // Return WC_Order objects directly
    ));
    
    // Cache for 2 minutes (invoices don't change frequently)
    set_transient($cache_key, $orders, 120);
    oj_debug_log('Invoice orders cached for table: ' . $table_number, 'INVOICE');
}
