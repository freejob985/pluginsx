...
// PERFORMANCE OPTIMIZED: Get products with caching and reasonable limit
$cache_key = 'oj_qr_menu_products_' . $table_id . '_' . ($woofood_location_id ?? 'all');
$products = get_transient($cache_key);

if ($products === false) {
    $product_args = array(
        'limit' => 200, // Optimized limit for QR menu display
        'status' => 'publish',
        'orderby' => 'menu_order',
        'order' => 'ASC'
    );
    ...
    oj_debug_log('QR menu products cached for table: ' . $table_number, 'QR_MENU');
}
...
<head>
    ...
    <!-- Orders Jet QR Menu Configuration -->
    <script type="text/javascript">
        window.OrdersJetQRMenu = {
            tableNumber: '<?php echo esc_js($table_number); ?>',
            tableId: <?php echo intval($table_id); ?>,
            ajaxUrl: '<?php echo esc_url(admin_url('admin-ajax.php')); ?>',
            currencySymbol: '<?php echo esc_js(get_woocommerce_currency_symbol()); ?>',
            nonces: {
                table_order: '<?php echo wp_create_nonce('oj_table_order'); ?>',
                call_waiter: '<?php echo wp_create_nonce('oj_call_waiter'); ?>',
                product_details: '<?php echo wp_create_nonce('oj_product_details'); ?>'
            }
        };
        
        // Debug: Log configuration on load
        console.log('Orders Jet QR Menu Configuration Loaded:', window.OrdersJetQRMenu);
        
        // Test configuration
        if (!window.OrdersJetQRMenu.tableNumber) {
            console.error('ERROR: Table number not set in configuration!');
        } else {
            console.log('✓ Table number configured:', window.OrdersJetQRMenu.tableNumber);
        }
        
        if (!window.OrdersJetQRMenu.ajaxUrl) {
            console.error('ERROR: AJAX URL not set in configuration!');
        } else {
            console.log('✓ AJAX URL configured:', window.OrdersJetQRMenu.ajaxUrl);
        }
    </script>
...
<body>
    <div class="container">
        ...
        <div id="menu-tab" class="tab-content">
            ...
            <div class="menu-grid" id="menu-grid">
                <?php 
                // PERFORMANCE FIX: Group products by category using bulk operations
                $products_by_category = array();
                
                if (!empty($products)) {
                    // Get all product IDs for bulk category fetching
                    $product_ids = array_map(function($product) {
                        return $product->get_id();
                    }, $products);
                    
                    // Use Menu Service bulk category fetching if available
                    if (class_exists('Orders_Jet_Menu_Service')) {
                        $menu_service = new Orders_Jet_Menu_Service();
                        $all_product_categories = $menu_service->get_products_categories_bulk($product_ids);
                        
                        foreach ($products as $product) {
                            $product_id = $product->get_id();
                            $product_categories = $all_product_categories[$product_id] ?? array();
                            
                            foreach ($product_categories as $cat_slug) {
                                if (!isset($products_by_category[$cat_slug])) {
                                    $products_by_category[$cat_slug] = array();
                                }
                                $products_by_category[$cat_slug][] = $product;
                            }
                        }
                    } else {
                        // Fallback: individual queries (less optimal)
                        foreach ($products as $product) {
                ...
                foreach ($categories as $category): 
                    $category_products = $products_by_category[$category->slug] ?? array();
                ?>
    ...
    <div class="call-waiter-btn" id="call-waiter-btn">
        <button class="waiter-call-button" id="waiter-call-button" title="<?php _e('Call Waiter for Assistance', 'orders-jet'); ?>">
            <span class="waiter-icon">🙋‍♂️</span>
            <span class="waiter-text"><?php _e('Call Waiter', 'orders-jet'); ?></span>
        </button>
    </div>
