...
class Orders_Jet_Table_Management {
    ...
    public function __construct() {
        ...
        oj_debug_log('Table Management class initialized with WooFood integration', 'TABLE_MGMT');
    }
    ...
    public function register_table_post_type() {
        ...
        if (post_type_exists('oj_table')) {
            oj_debug_log('Table post type registered successfully', 'TABLE_MGMT');
        } else {
            oj_error_log('Failed to register table post type', 'TABLE_MGMT');
        }
    }
    ...
    public function register_table_taxonomies() {
        ...
        if (taxonomy_exists('oj_table_zone')) {
            oj_debug_log('Table zone taxonomy registered successfully', 'TABLE_MGMT');
        } else {
            oj_error_log('Failed to register table zone taxonomy', 'TABLE_MGMT');
        }
    }
    ...
    public function save_table_meta($post_id) {
        oj_debug_log("save_table_meta called for post ID: {$post_id}", 'TABLE_META');
        
        if (get_post_type($post_id) !== 'oj_table') {
            oj_debug_log("Post type is not oj_table, skipping", 'TABLE_MANAGEMENT');
            return;
        }
        
        if (!isset($_POST['oj_table_meta_nonce']) || !wp_verify_nonce($_POST['oj_table_meta_nonce'], 'oj_table_meta')) {
            oj_debug_log("Nonce verification failed", 'TABLE_META');
            return;
        }
        ...
        if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) {
            oj_debug_log("Autosave in progress, skipping", 'TABLE_META');
            return;
        }
        
        if (!current_user_can('edit_post', $post_id)) {
            oj_debug_log("User doesn't have permission to edit post", 'TABLE_META');
            return;
        }
        
        oj_debug_log("All checks passed, proceeding with save", 'TABLE_META');
        ...
        foreach ($meta_fields as $form_field => $meta_key) {
            if (isset($_POST[$form_field])) {
                ...
                oj_debug_log("Saving meta field - {$form_field} => {$meta_key} = {$value}", 'TABLE_META');
            } else {
                oj_debug_log("Form field not found - {$form_field}", 'TABLE_META');
            }
        }
        
        // Generate QR code if table number is set
        $table_number = get_post_meta($post_id, '_oj_table_number', true);
        oj_debug_log("Table number retrieved: {$table_number}", 'TABLE_QR');
        
        if ($table_number) {
            oj_debug_log("Generating QR code for table: {$table_number}", 'TABLE_QR');
            $qr_code_url = $this->generate_qr_code($post_id, $table_number);
            oj_debug_log("QR code generated: {$qr_code_url}", 'TABLE_QR');
        } else {
            oj_debug_log("No table number found, skipping QR code generation", 'TABLE_QR');
        }
    }
    ...
    public function generate_qr_code($table_id, $table_number) {
        ...
        oj_debug_log("QR URL: {$qr_url}", 'TABLE_QR');
        ...
        oj_debug_log("QR Code URL: {$qr_code_url}", 'TABLE_QR');
        
        // Save QR code URL to meta
        $saved = update_post_meta($table_id, '_oj_table_qr_code', $qr_code_url);
        oj_debug_log("QR code meta saved: " . ($saved ? 'true' : 'false'), 'TABLE_QR');
