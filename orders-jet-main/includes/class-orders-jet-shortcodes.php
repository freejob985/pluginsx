...
class Orders_Jet_Shortcodes {
    ...
    public function table_list_shortcode($atts) {
        ...
        // Use centralized query service for table list shortcode
        $options = array(
            'limit' => -1,
            'orderby' => 'meta_value',
            'order' => 'ASC',
            'cache' => true,
            'cache_duration' => 300, // 5 minutes cache for shortcode
            'meta_query' => array()
        );
        
        // Add zone filter if specified
        if (!empty($atts['zone'])) {
            // Note: tax_query needs to be handled differently in centralized service
            // For now, we'll add it to meta_query and handle in the service
