...
class Orders_Jet_Individual_Order_Completion_Handler {
    ...
    public function __construct($tax_service, $notification_service = null) {
    ...
    private function process_order_completion($order, $payment_method) {
        ...
        $this->notification_service->store_dashboard_notification(array(
            ...
            'message' => sprintf(__('Order #%s completed - Payment: %s', 'orders-jet'), $order->get_order_number(), $payment_method)
        ));
    ...
    private function clear_orders_master_cache() {
        ...
        oj_debug_log('Orders Master cache cleared after order completion', 'CACHE_CLEAR');
    }
    ...
    private function generate_completion_response($order, $payment_method, $original_total) {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            oj_debug_log('Individual order #' . $order_id . ' completed - Original Total: ' . $original_total . ', New Total with Tax: ' . $order->get_total(), 'ORDER_COMPLETE');
        }
