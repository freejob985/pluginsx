...
class Orders_Jet_Product_Details_Handler {
    ...
    private function get_product_variations($product) {
        ...
        try {
            oj_debug_log('Processing variable product with ID: ' . $product->get_id(), 'PRODUCT_DETAILS');
            
            // Use WooCommerce's get_available_variations() method
            $available_variations = $product->get_available_variations();
            oj_debug_log('Found ' . count($available_variations) . ' available variations', 'PRODUCT_DETAILS');
            
            // Get attributes
            $attributes = $product->get_variation_attributes();
            oj_debug_log('Product attributes: ' . print_r($attributes, true), 'PRODUCT_DETAILS');
            
            foreach ($attributes as $attribute_name => $options) {
                $attribute_label = wc_attribute_label($attribute_name);
                $variation_options = array();
                
                oj_debug_log('Processing attribute: ' . $attribute_name . ' with options: ' . implode(', ', $options), 'PRODUCT_DETAILS');
               ...
               oj_debug_log('MATCH! Found variation for ' . $option . ' - Price: ' . $variation_price . ', ID: ' . $variation_id, 'PRODUCT_DETAILS');
                                break 2;
                            }
                        }
                    }
       ...
       oj_debug_log('No variation found for attribute ' . $attribute_name . ' = ' . $option, 'PRODUCT_DETAILS');
                    }
            ...
            oj_error_log('Error processing variations: ' . $e->getMessage(), 'PRODUCT_DETAILS');
            $variations = array();
        }
