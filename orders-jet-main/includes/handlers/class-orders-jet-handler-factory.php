...
class Orders_Jet_Handler_Factory {
    ...
    /**
     * Get Bulk Actions Handler instance
     * 
     * @since 2.0.0
     */
    public function get_bulk_actions_handler() {
        if (!isset(self::$instances['bulk_actions'])) {
            self::$instances['bulk_actions'] = new Orders_Jet_Bulk_Actions_Handler($this);
        }
        return self::$instances['bulk_actions'];
    }
    
    /**
     * Get Kitchen Refresh Handler instance
     * Note: Now returns dashboard refresh handler since we use simple client-side refresh
     * 
     * @since 2.0.0
     */
    public function get_kitchen_refresh_handler() {
        // Use dashboard refresh handler for backward compatibility
        return $this->get_dashboard_refresh_handler();
    }
    
    /**
     * Get Dashboard Refresh Handler instance
     * 
     * @since 2.0.0
     */
    public function get_dashboard_refresh_handler() {
        if (!isset(self::$instances['dashboard_refresh'])) {
            $order_method_service = new Orders_Jet_Order_Method_Service();
            self::$instances['dashboard_refresh'] = new Orders_Jet_Dashboard_Refresh_Handler(
                $this->kitchen_service,
                $order_method_service
            );
        }
        return self::$instances['dashboard_refresh'];
    }
    
    /**
     * Get Order Details Handler instance
     * 
     * @since 2.0.0
     */
    public function get_order_details_handler() {
        if (!isset(self::$instances['order_details'])) {
            self::$instances['order_details'] = new Orders_Jet_Order_Details_Handler(
                $this->kitchen_service,
                $this->tax_service
            );
        }
        return self::$instances['order_details'];
    }
    
    /**
     * Get Filter Views Handler instance
     * 
     * @since 2.0.0
     */
    public function get_filter_views_handler() {
        if (!isset(self::$instances['filter_views'])) {
            self::$instances['filter_views'] = new Orders_Jet_Filter_Views_Handler();
        }
        return self::$instances['filter_views'];
    }
    
    /**
     * Get Order Editor Handler instance
     * 
     * @return Orders_Jet_Order_Editor_Handler
     * @since 2.0.0
     */
    public function get_order_editor_handler() {
        if (!isset(self::$instances['order_editor'])) {
            // Create handler with dependencies from constructor
            self::$instances['order_editor'] = new Orders_Jet_Order_Editor_Handler(
                $this->kitchen_service,
                $this->notification_service,
                $this->tax_service
            );
        }
        return self::$instances['order_editor'];
    }
    
    /**
     * Get Table Assignment Handler instance
     */
    public function get_table_assignment_handler() {
        if (!isset(self::$instances['table_assignment'])) {
            self::$instances['table_assignment'] = new Orders_Jet_Table_Assignment_Handler(
                $this->notification_service
            );
        }
        return self::$instances['table_assignment'];
    }
