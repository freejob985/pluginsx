...
class OJ_Delivery_Time_Manager {
    ...
    public static function set_delivery_time($order, $date_time, $type = 'pickup') {
        ...
        if (!$timestamp) {
            oj_error_log("Invalid date_time format: {$date_time}", 'DELIVERY_TIME');
            return false;
        }
        ...
        // Log the action
        oj_debug_log("Set {$type} time for Order #{$order->get_id()}: {$normalized_datetime} (timestamp: {$timestamp})", 'DELIVERY_TIME');
    ...
    public static function import_woofood_time($order) {
        ...
        if ($delivery_info) {
            ...
            oj_debug_log("Imported WooFood time for Order #{$order->get_id()}: {$delivery_info['datetime']}", 'DELIVERY_TIME');
            return true;
        }
