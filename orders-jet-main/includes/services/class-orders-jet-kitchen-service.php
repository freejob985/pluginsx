...
class Orders_Jet_Kitchen_Service {
    ...
    public function mark_kitchen_ready($order, $kitchen_type) {
        if (!in_array($kitchen_type, ['food', 'beverages'])) {
            oj_error_log('Invalid kitchen type: ' . $kitchen_type, 'KITCHEN_SERVICE');
            return false;
        }
        ...
        oj_debug_log('Marked ' . $kitchen_type . ' ready for order #' . $order->get_id(), 'KITCHEN_SERVICE');
        return true;
    }
    ...
    public function get_filtered_kitchen_orders($kitchen_filter = 'all', $status_filter = 'all') {
        // Get orders that are in processing status - OPTIMIZED with reasonable limit
        $orders = wc_get_orders(array(
            'status' => 'processing',
            'limit' => 200, // Reasonable limit for kitchen service
            'orderby' => 'date',
            'order' => 'ASC'
        ));
