...
class Orders_Jet_Tax_Service {
    ...
    public function calculate_individual_order_taxes($order) {
        ...
        if (!$tax_enabled) {
            oj_debug_log('Taxes not enabled in WooCommerce', 'TAX_CONFIG');
            return;
        }
        ...
        foreach ($order->get_items() as $item_id => $item) {
            ...
            if (!empty($tax_rates)) {
                ...
                oj_debug_log("Applied individual order taxes to item: {$item->get_name()}", 'TAX_CALC', array(
                    'tax_amount' => array_sum($line_taxes)
                ));
            }
        }
        ...
        oj_debug_log("Individual Order tax calculation", 'TAX_CALC', array(
    ...
    public function calculate_table_invoice_taxes($subtotal, $order_ids) {
        if (!wc_tax_enabled()) {
            oj_debug_log('Taxes not enabled - returning zero tax amounts', 'TAX_CONFIG');
            return array(
                'service_tax' => 0,
                'vat_tax' => 0,
                'total_tax' => 0,
        ...
        if (empty($tax_rates)) {
            oj_debug_log('No tax rates found for location - returning zero tax amounts', 'TAX_CONFIG');
            return array(
                'service_tax' => 0,
                'vat_tax' => 0,
                'total_tax' => 0,
        ...
        oj_debug_log('Table invoice tax calculation', 'TAX_CALC', array(
            'subtotal' => $subtotal,
        ...
        switch ($expected_tax_behavior) {
            case 'deferred':
                ...
                if ($is_table_order && $order_tax > 0.01) {
                    oj_error_log("TAX ISOLATION VIOLATION - Table order #{$order->get_id()} has individual tax: {$order_tax} (should be 0)", 'TAX_VALIDATION');
                    return false;
                }
                break;
            ...
            case 'calculated':
                ...
                if (!$is_table_order && $order_tax <= 0) {
                    oj_error_log("TAX ISOLATION VIOLATION - Pickup order #{$order->get_id()} has no tax: {$order_tax} (should be > 0)", 'TAX_VALIDATION');
                    return false;
                }
                break;
            ...
            case 'consolidated':
                ...
                if (empty($session_id)) {
                    oj_error_log("TAX ISOLATION WARNING - Consolidated order #{$order->get_id()} missing session ID", 'TAX_VALIDATION');
                    return false;
                }
                break;
        }
        
        oj_debug_log("Tax isolation validation PASSED", 'TAX_VALIDATION', array(
            'order_id' => $order->get_id(),
            'expected_behavior' => $expected_tax_behavior,
